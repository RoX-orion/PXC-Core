...
#define GET_VERBOSITY_LEVEL() (::pxc::get_verbosity_level())
#define SET_VERBOSITY_LEVEL(new_level) (::pxc::set_verbosity_level(new_level))
...
#define LOGGER(interface, options, level, comment) ::pxc::Logger(interface, options, level, __FILE__, __LINE__, comment)
...
#define LOG_IMPL_FULL(interface, options, strip_level, runtime_level, condition, comment) \
  LOG_IS_STRIPPED(strip_level) || runtime_level > options.get_level() || !(condition)     \
      ? (void)0                                                                           \
      : ::pxc::detail::Voidify() & LOGGER(interface, options, runtime_level, comment)

#define LOG_IMPL(strip_level, level, condition, comment) \
  LOG_IMPL_FULL(*::pxc::log_interface, ::pxc::log_options, strip_level, VERBOSITY_NAME(level), condition, comment)

#define LOG(level) LOG_IMPL(level, level, true, ::pxc::Slice())
...
#define LOG_TAG ::pxc::Logger::tag_
#define LOG_TAG2 ::pxc::Logger::tag2_
