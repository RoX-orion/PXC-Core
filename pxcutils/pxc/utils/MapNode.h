...
struct MapNode<KeyT, ValueT, EqT, typename std::enable_if_t<(sizeof(KeyT) + sizeof(ValueT) > 28 * sizeof(void *))>> {
  ...
  MapNode(KeyT key, ValueT value) : impl_(pxc::make_unique<Impl>(std::move(key), std::move(value))) {
  }

  void copy_from(const MapNode &other) {
    DCHECK(empty());
    DCHECK(!other.empty());
    impl_ = pxc::make_unique<Impl>(other.impl_->first, other.impl_->second);
  }
  ...
  void emplace(KeyT key, ArgsT &&...args) {
    DCHECK(empty());
    impl_ = pxc::make_unique<Impl>(std::move(key), std::forward<ArgsT>(args)...);
  }
};
