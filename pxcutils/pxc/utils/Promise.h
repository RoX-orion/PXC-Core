...
#include "pxc/utils/CancellationToken.h"
#include "pxc/utils/common.h"
#include "pxc/utils/invoke.h"
...
auto promise_interface_ptr(F &&f) {
  return pxc::make_unique<std::decay_t<decltype(promise_interface<T>(std::forward<F>(f)))>>(
...
class PromiseCreator {
 public:
  ...
  static Promise<ArgT> lambda(OkT &&ok) {
    return Promise<ArgT>(pxc::make_unique<detail::LambdaPromise<ArgT, std::decay_t<OkT>>>(std::forward<OkT>(ok)));
  }

  template <class OkT, class ArgT = detail::drop_result_t<detail::get_arg_t<OkT>>>
  static auto cancellable_lambda(CancellationToken cancellation_token, OkT &&ok) {
    return Promise<ArgT>(pxc::make_unique<detail::CancellablePromise<detail::LambdaPromise<ArgT, std::decay_t<OkT>>>>(
        std::move(cancellation_token), std::forward<OkT>(ok)));
  }

  template <class... ArgsT>
  static Promise<> join(ArgsT &&...args) {
    return Promise<>(pxc::make_unique<detail::JoinPromise<ArgsT...>>(std::forward<ArgsT>(args)...));
  }
};
