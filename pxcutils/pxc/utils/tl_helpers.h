...
#include "pxc/utils/FlatHashSet.h"
...
#include "pxc/utils/tl_storers.h"
#include "pxc/utils/unique_value_ptr.h"
#include "pxc/utils/Variant.h"
...
#define BEGIN_STORE_FLAGS()       \
  do {                            \
    ::pxc::uint32 flags_store = 0; \
  ::pxc::uint32 bit_offset_store = 0
...
#define END_STORE_FLAGS()           \
  ...
  ::pxc::store(flags_store, storer); \
  }                                 \
  while (false)

#define BEGIN_PARSE_FLAGS()            \
  do {                                 \
    ::pxc::uint32 flags_parse;          \
    ::pxc::uint32 bit_offset_parse = 0; \
  ::pxc::parse(flags_parse, parser)
