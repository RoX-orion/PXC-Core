...
#define ASSERT_EQ(expected, received) ::pxc::assert_eq_impl((expected), (received), __FILE__, __LINE__)

#define ASSERT_TRUE(received) ::pxc::assert_true_impl((received), __FILE__, __LINE__)

#define ASSERT_STREQ(expected, received) \
  ::pxc::assert_eq_impl(::pxc::Slice((expected)), ::pxc::Slice((received)), __FILE__, __LINE__)

#define REGRESSION_VERIFY(data) ::pxc::TestContext::get()->verify(data).ensure()
...
#define TEST_IMPL(test_name)                                                                                         \
  class test_name final : public ::pxc::Test {                                                                        \
   public:                                                                                                           \
    using Test::Test;                                                                                                \
    void run() final;                                                                                                \
  };                                                                                                                 \
  ::pxc::RegisterTest<test_name> TD_CONCAT(test_instance_, TD_CONCAT(test_name, __LINE__))(TD_DEFINE_STR(test_name)); \
  void test_name::run()
