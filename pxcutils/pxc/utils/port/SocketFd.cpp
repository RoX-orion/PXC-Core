...
#include "pxc/utils/port/detail/Iocp.h"
#include "pxc/utils/port/Mutex.h"
#include "pxc/utils/VectorQueue.h"
...
class SocketFdImpl final : private Iocp::Callback {
 public:
  ...
  Result<size_t> read(MutableSlice slice) {
    ...
    auto res = input_reader_.advance(pxc::min(slice.size(), input_reader_.size()), slice);
    if (res == 0) {
