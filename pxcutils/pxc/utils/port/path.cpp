...
#include "pxc/utils/port/detail/skip_eintr.h"
#include "pxc/utils/ScopeGuard.h"
#include "pxc/utils/SliceBuilder.h"

#if TD_PORT_WINDOWS
#include "pxc/utils/port/FromApp.h"
#include "pxc/utils/port/wstring_convert.h"
...
Status mkdir(CSlice dir, int32 mode) {
  ...
  auto status = pxc::CreateDirectoryFromAppW(wdir.c_str(), nullptr);
...
Status rename(CSlice from, CSlice to) {
  ...
  auto status = pxc::MoveFileExFromAppW(wfrom.c_str(), wto.c_str(), MOVEFILE_REPLACE_EXISTING);
  if (status == 0) {
    return OS_ERROR(PSLICE() << "Can't rename \"" << from << "\" to \"" << to << '\"');
  }
  return Status::OK();
...
Status rmdir(CSlice dir) {
  TRY_RESULT(wdir, to_wstring(dir));
  int status = pxc::RemoveDirectoryFromAppW(wdir.c_str());
  if (!status) {
    return OS_ERROR(PSLICE() << "Can't delete directory \"" << dir << '"');
  }
  return Status::OK();
...
Status unlink(CSlice path) {
  ...
  int status = pxc::DeleteFileFromAppW(wpath.c_str());
  if (!status) {
    return OS_ERROR(PSLICE() << "Can't unlink \"" << path << '"');
  }
  return Status::OK();
...
static Result<bool> walk_path_dir(const std::wstring &dir_name,
  ...
  auto handle =
      pxc::FindFirstFileExFromAppW(name.c_str(), FindExInfoStandard, &file_data, FindExSearchNameMatch, nullptr, 0);
