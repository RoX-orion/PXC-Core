...
inline std::string string_to_unmanaged(String ^ str) {
  ...
  auto r_unmanaged_str = pxc::from_wstring(str->Data(), str->Length());
...
inline String ^ string_from_unmanaged(const std::string &from) {
  auto tmp = pxc::to_wstring(from).ok();
...
using uint8 = pxc::uint8;
using int32 = pxc::int32;
using int64 = pxc::int64;
