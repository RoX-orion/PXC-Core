...
Result<IPAddress> IPAddress::get_ipv4_address(CSlice host) {
  ...
  host = ::pxc::get_ip_str(AF_INET, &ipv4_numeric_addr);
  IPAddress result;
...
string IPAddress::ipv4_to_str(uint32 ipv4) {
  ipv4 = ntohl(ipv4);
  return ::pxc::get_ip_str(AF_INET, &ipv4).str();
}

string IPAddress::ipv6_to_str(Slice ipv6) {
  CHECK(ipv6.size() == 16);
  return ::pxc::get_ip_str(AF_INET6, ipv6.ubegin()).str();
}
...
CSlice IPAddress::get_ip_str() const {
  ...
  switch (get_address_family()) {
    case AF_INET6:
      return ::pxc::get_ip_str(AF_INET6, &ipv6_addr_.sin6_addr);
    case AF_INET:
      return ::pxc::get_ip_str(AF_INET, &ipv4_addr_.sin_addr);
    default:
      UNREACHABLE();
      return CSlice();
  }
...
string IPAddress::get_ip_host() const {
  ...
  switch (get_address_family()) {
    case AF_INET6:
      return PSTRING() << '[' << ::pxc::get_ip_str(AF_INET6, &ipv6_addr_.sin6_addr) << ']';
    case AF_INET:
      return ::pxc::get_ip_str(AF_INET, &ipv4_addr_.sin_addr).str();
    default:
      UNREACHABLE();
      return string();
  }
