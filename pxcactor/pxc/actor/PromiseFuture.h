...
inline Promise<Unit> create_event_promise(EventFull &&ok) {
  return Promise<Unit>(pxc::make_unique<detail::EventPromise>(std::move(ok)));
}

inline Promise<Unit> create_event_promise(EventFull ok, EventFull fail) {
  return Promise<Unit>(pxc::make_unique<detail::EventPromise>(std::move(ok), std::move(fail)));
}
...
Promise<T> create_promise_from_promise_actor(PromiseActor<T> &&from) {
  return Promise<T>(pxc::make_unique<PromiseActor<T>>(std::move(from)));
}
